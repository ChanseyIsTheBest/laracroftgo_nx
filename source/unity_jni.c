...
#include <string.h>
#include <stdint.h>
#include "config.h"   /* GAME_PACKAGE, GAME_FOLDER */
#include "asset_pack.h"  /* assets live in the pack after first boot */
#include <fcntl.h>
#include <unistd.h>
#include <sys/stat.h>
#include <errno.h>
...
static char g_root[192];            /* e.g. /switch/lcgo                  */
...
enum { UJ_INPUTSTREAM, UJ_AFD, UJ_FD, UJ_PREFS, UJ_EDITOR, UJ_GENERIC,
       UJ_MAP, UJ_SET, UJ_ITER, UJ_ENTRY, UJ_BOXED, UJ_SCANNER };
...
typedef struct {
  ...
  void *src;                /* UJ_SCANNER: backing InputStream handle       */
  char  name[64];           /* UJ_INPUTSTREAM: asset name, for boot.config   */
} UHandle;
...
/* java.util.Scanner over an InputStream.
 *
 * WHY THIS EXISTS. Unity reads assets/bin/Data/boot.config on Android through
 * the Java API, not through libc:
 *
 *     getAssets().open("bin/Data/boot.config")     -> InputStream
 *     new Scanner(stream, "UTF-8").useDelimiter("\\A").next()
 *
 * Scanner was not implemented anywhere in this port, so `useDelimiter` returned
 * Ljava/util/Scanner; -> fell through act_object -> NULL, and `next()` returned
 * the generic "" fallback. The engine therefore booted with an EMPTY
 * boot.config on every run. The tell in debug.log is Unity looking up
 * java/util/NoSuchElementException immediately after the next() method id --
 * that is the exception Scanner.next() throws when the input is empty, and
 * Unity only reaches for it on the failure path.
 *
 * The `\A` delimiter idiom means "one token = the entire stream", which is why
 * next() below simply returns everything remaining. That is exact for this
 * call site. A Scanner used for real tokenising would need actual delimiter
 * handling -- if one ever shows up, the [jni] UNMATCHED log will name it.
 *
 * The handle borrows the InputStream's FILE* rather than reopening, so reads
 * advance the shared stream exactly as Java's does. */
static int is_uh(void *p,int kind);   /* defined below */
void *unity_make_scanner(void *inputstream){
  UHandle *src = inputstream;
  UHandle *h = uh_new(UJ_SCANNER);
  h->src = (is_uh(src,UJ_INPUTSTREAM)) ? src : NULL;
  debugPrintf("[jni] new Scanner(InputStream=%p) -> %s\n", inputstream,
              h->src ? "backed" : "EMPTY (arg was not an InputStream)");
  return h;
}
 ...
 * getters below). It must be Unix-rooted ("/switch/lcgo"): Mono/IL2CPP on
 ...
 * relative asset path -> "assets/bin/Data/sdmc:/switch/<game>". newlib then
...
void *unity_dispatch_object(void *recv, const void *id_, va_list va){ const struct FakeID *id = id_;
  ...
  /* java.util.Scanner -- see unity_make_scanner() for why this exists. */
  if (has(cls,"java/util/Scanner")){
    UHandle *h = recv;
    /* useDelimiter/reset return `this` in Java; the boot.config idiom chains
     * straight off them, so returning anything else (including NULL, which is
     * what happened before) breaks the chain immediately. */
    if (has(m,"useDelimiter") || has(m,"reset") || has(m,"skip")) return recv;
    if (has(m,"next") || has(m,"nextLine")){
      UHandle *src = is_uh(h,UJ_SCANNER) ? h->src : NULL;
      FILE *fp = src ? src->fp : NULL;
      if (!fp || h->idx){                 /* exhausted: Java would throw here */
        debugPrintf("[jni] Scanner.%s -> \"\" (%s)\n", m,
                    fp ? "already consumed" : "no backing stream");
        return jni_make_string("");
      }
      long cur = ftell(fp);
      fseek(fp,0,SEEK_END); long end = ftell(fp); fseek(fp,cur,SEEK_SET);
      long n = end - cur;
      if (n <= 0){ h->idx = 1; debugPrintf("[jni] Scanner.%s -> \"\" (empty)\n", m);
                   return jni_make_string(""); }
      char *buf = malloc((size_t)n + 1);
      if (!buf) return jni_make_string("");
      size_t got = fread(buf,1,(size_t)n,fp);
      buf[got] = 0;
      h->idx = 1;                          /* one token, per the \A delimiter */
      debugPrintf("[jni] Scanner.%s -> %zu bytes\n", m, got);

      /* ------------------------------------------------------------------
       * boot.config worker-count cap.
       *
       * WHY. On the first hardware boot of this port the engine hung forever
       * with a black screen. The watchdog showed UnityMain parked in
       * pthread_cond_wait inside PlatformThread::Create (libunity+0x5e3d78),
       * called from JobSystem::CreateJobSystem -> BackgroundJobQueue ->
       * JobQueue::JobQueue -> ujob_init, with a thread list that was
       * byte-identical across 23 watchdog dumps over 140 s:
       *     UnityMain, 2x AssetGarbageCollectorHelper, GC Finalizer,
       *     Job.Worker 0..1, Background Job.Worker 0..8      (15 threads)
       * i.e. 9 background workers had started and the NEXT thread never
       * appeared. PlatformThread::Create discards pthread_create's return
       * value and then waits on a condvar for the child to set a "started"
       * flag, so a creation that fails is an unbreakable hang with no error.
       *
       * Memory was NOT the constraint (64 MB of a 512 MB overcommit pool in
       * use), which points at a per-process thread ceiling instead. Unity
       * sizes this pool itself and it has no idea it is on a console.
       *
       * Unity supports capping it from boot.config; both keys are present as
       * literals in THIS libunity.so. Appending wins over an earlier value
       * because the parser takes the last occurrence, so we never have to edit
       * the user's assets -- their real boot.config still loads first.
       *
       * 3 background + 2 job workers matches the 3 cores this port reports via
       * sysconf/proc/cpuinfo and drops the engine's thread demand by 6.
       * ------------------------------------------------------------------ */
      if (src && src->name[0] && strstr(src->name, "boot.config")) {
        static const char kCap[] =
            "\nbackground-job-worker-count=3"
            "\njob-worker-count=2\n";
        char *cfg = malloc(got + sizeof kCap + 1);
        if (cfg) {
          memcpy(cfg, buf, got);
          memcpy(cfg + got, kCap, sizeof kCap);   /* includes the NUL */
          debugPrintf("[jni] boot.config: appended worker caps "
                      "(background-job-worker-count=3, job-worker-count=2) "
                      "-- %zu -> %zu bytes\n", got, got + sizeof kCap - 1);
          free(buf);
          void *rc = jni_make_string(cfg);
          free(cfg);
          return rc;
        }
      }

      void *r = jni_make_string(buf);
      free(buf);
      return r;
    }
  }
  ...
  if (has(cls,"AssetManager")){
    if (has(m,"openFd") || has(m,"openNonAssetFd")){
      ...
      /* These are raw libc calls, so they bypass libc_shim's pack hooks. After
       * the first boot the loose tree is gone and the pack is the only copy --
       * observed as "AssetManager.open(bin/Data/boot.config) -> MISSING". */
      int fd = asset_pack_open_path(path);
    ...
    if (has(m,"open")){
      ...
      char path[320]; asset_path(path,sizeof path,name);
      FILE *fp = NULL;
      {   /* pack first: see the openFd note above */
        void *data = NULL; size_t sz = 0;
        if (asset_pack_read_all_path(path, &data, &sz))
          fp = fmemopen(data, sz ? sz : 1, "rb");
      }
      if (!fp) fp = fopen(path,"rb");
      debugPrintf("[io] JNI AssetManager.open(%s) -> %s [%s]\n", name, fp?"ok":"MISSING", path);
      if(!fp) return NULL;
      UHandle*h=uh_new(UJ_INPUTSTREAM); h->fp=fp;
      /* remember the asset name so Scanner.next() can recognise boot.config
       * and append the worker-count caps -- see the note there. */
      if (name) { strncpy(h->name, name, sizeof h->name - 1); h->name[sizeof h->name - 1] = 0; }
      return h;
    }
...
uint64_t unity_dispatch_int(void *recv, const void *id_, va_list va){ const struct FakeID *id = id_;
  ...
  /* Scanner.hasNext()/hasNextLine(): true while the backing stream has bytes
   * left and no token has been consumed yet. */
  if (has(cls,"java/util/Scanner")){
    UHandle *h = recv;
    if (has(m,"hasNext")){
      UHandle *src = is_uh(h,UJ_SCANNER) ? h->src : NULL;
      if (!src || !src->fp || h->idx) return 0;
      long cur=ftell(src->fp); fseek(src->fp,0,SEEK_END);
      long end=ftell(src->fp); fseek(src->fp,cur,SEEK_SET);
      return (uint64_t)(end>cur ? 1 : 0);
    }
  }
...
void unity_dispatch_void(void *recv, const void *id_, va_list va){ const struct FakeID *id = id_;
  const char *cls=id->cls, *m=id->name;
  /* Scanner.close() closes the underlying stream, as Java's does. Guarded so a
   * double close (Scanner then InputStream) cannot fclose twice. */
  if (has(cls,"java/util/Scanner") && has(m,"close")){
    UHandle *h=recv; UHandle *src = is_uh(h,UJ_SCANNER) ? h->src : NULL;
    if (src && src->fp){ fclose(src->fp); src->fp=NULL; }
    if (is_uh(h,UJ_SCANNER)) h->idx = 1;
    return;
  }
