 ...
 * /switch/<game>/jni_trace.log so you can pull it off the SD card.
 *
...
#include <stdio.h>
#include "config.h"   /* GAME_FOLDER -- the trace log must land in OUR data dir */
...
static inline void jni_trace(const char *what, const char *cls,
  ...
  static int   tried = 0;
  if (!log && !tried){ tried = 1; log = fopen(game_path("jni_trace.log"), "a"); }
